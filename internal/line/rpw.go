...
// RPWBalance assigns tasks to stations using the Ranked Positional Weight
// heuristic. Tasks are sorted by descending RPW and assigned to the first
// station that has remaining capacity, respecting precedence.
func RPWBalance(g *task.Graph, cycleTime float64) ([]Station, error) {
	...
	return leakPreviousStations(stations), nil
}

// assignWithPrecedence assigns tasks in the given order to stations, respecting
// precedence constraints. A task can only be assigned after all its predecessors
// have been assigned.
func assignWithPrecedence(g *task.Graph, order []string, cycleTime float64) []Station {
	...
	for len(remaining) > 0 {
		...
		for _, id := range remaining {
			...
			// Try to fit in an existing station.
			placed := false
		...
		if !progress {
			// Remaining tasks have unmet precedence or exceed cycle time; open
			// new stations for them to avoid infinite loop.
			for _, id := range next {
				t := g.Get(id)
				stations = append(stations, Station{Tasks: []string{id}, Load: t.Time})
				assigned[id] = true
