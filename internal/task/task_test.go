...
func TestCriticalPath(t *testing.T) {
	...
	// Critical path: A(5)->C(6)->D(7)->G(5)->H(3) = 26
	// or A(5)->B(3)->D(7)->G(5)->H(3) = 23, so path through C is longer.
	if dur != 26 {
...
func TestPositionalWeight(t *testing.T) {
	...
	// RPW(A) should be the largest since all tasks are reachable from A.
	// RPW(A) = 5+3+6+7+4+2+5+3 = 35
	if rpw["A"] != 35 {
		t.Fatalf("RPW(A) = %v, want 35", rpw["A"])
	}
	// RPW(H) = 3 (leaf, just its own time).
	if rpw["H"] != 3 {
...
func TestTotalFloat(t *testing.T) {
	...
	// E is not on the critical path: A->B->E->H = 5+3+4+3 = 15, slack = 26-15 = 11?
	// Actually: ES(E) = ES(B)+B.Time = 5+3=8, EF(E)=12.
	// LS(E) = LF(E)-4. LF(E)=min(LS(H))=23. LS(E)=19. Float=19-8=11.
	if floats["E"] != 11 {
