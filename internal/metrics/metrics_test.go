...
func TestDominates(t *testing.T) {
	...
	// a has fewer stations, likely dominates in stations and smoothness.
	if !Dominates(a, b, objs) && !Dominates(b, a, objs) {
		// Neither dominates: that's also valid for this test set.
...
func TestLaborProductivity(t *testing.T) {
	...
	// 400 units / (4 workers * 8 hours) = 12.5 units/labor-hour
